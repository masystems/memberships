...
def reports(request, title, report, file_type):
    ...
    if file_type == 'xlsx':
        ...
        if report == 'mailing-list':
            ...
            columns = ['Title', 'First Name', 'Surname', 'Company Name', 'Address 1',
            ...
            for subscription in subscriptions:
                ...
                if subscription.active:
                    ...
                    if mail:
                        ...
                        # company name
                        worksheet.write(row_num, 3, subscription.member.company, font_style)
                        # address 1
                        worksheet.write(row_num, 4, subscription.member.address_line_1, font_style)
                        # address 2
                        worksheet.write(row_num, 5, subscription.member.address_line_2, font_style)
                        # address 3 (town)
                        worksheet.write(row_num, 6, subscription.member.town, font_style)
                        # address 4 (county)
                        worksheet.write(row_num, 7, subscription.member.county, font_style)
                        # postcode
                        worksheet.write(row_num, 8, subscription.member.postcode, font_style)
                        # country
                        worksheet.write(row_num, 9, subscription.member.country, font_style)
...
def export_payments_detailed(request, title):
    if request.POST:
        ...
        if 'csv' in request.POST:
            ...
            headers = get_payments_detailed_headers()

            writer.writerow(headers)
            for payment in all_payments.all():
                inbuilt_items = get_inbuilt_payment_items(payment, membership_package)
                writer.writerow(inbuilt_items)
            return response
...
def get_payments_detailed_headers():
    # column header names
    # member_number, name, email etc
    headers = ['Payment ID',
               'Name',
...
def get_inbuilt_payment_items(payment, membership_package):
